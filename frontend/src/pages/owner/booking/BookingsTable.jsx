...
const BookingsTable = ({ bookings }) => {
  ...
  useEffect(() => {
    const fetchSitterNames = async () => {
      const names = {};
      for (const booking of bookings) {
        if (booking.sitter_id && !names[booking.sitter_id]) {
          try {
            const sitter = await getSitterProfile(booking.sitter_id);
            names[booking.sitter_id] = sitter.display_name;
          } catch (error) {
            console.error(`Error fetching sitter ${booking.sitter_id}:`, error);
            names[booking.sitter_id] = `Sitter #${booking.sitter_id}`;
          }
        }
      }
      setSitterNames(names);
    };
